...
const T = {
  ...
  login_greeting:     { 'en-us':'Welcome, ','en-uk':'Welcome, ','ja':'ようこそ、','es':'Bienvenido, ','zh':'欢迎，' },
};
...
// =============================================
// DATA STORE (in-memory, multi-language)
// =============================================
let newsData = [
  {
    id: 1,
...
// About content per language (admin edits only affect ID base; translations are static)
const aboutTranslations = {
  'en-us': `<h2>About Demo.Website</h2>
<p>Demo.Website is a trusted digital news portal dedicated to providing the latest, accurate, and balanced information to readers across Indonesia and beyond.</p>
<h3>Our Vision</h3>
<p>To become a trusted source of information that empowers society through high-quality and responsible journalism.</p>
<h3>Our Mission</h3>
<p>We are committed to presenting verified news, upholding the principles of honesty, and providing space for diverse perspectives in every report.</p>
<h3>Editorial Team</h3>
<p>Strengthened by a team of experienced journalists with backgrounds in journalism and various fields of expertise, we are available 24/7 to ensure you never miss a single important story.</p>`,
  'en-uk': `<h2>About Demo.Website</h2>
<p>Demo.Website is a trusted digital news portal dedicated to providing the latest, accurate, and balanced information to readers across Indonesia and beyond.</p>
<h3>Our Vision</h3>
<p>To become a trusted source of information that empowers society through high-quality and responsible journalism.</p>
<h3>Our Mission</h3>
...
<h3>Editorial Team</h3>
<p>Strengthened by a team of experienced journalists with backgrounds in journalism and various fields of expertise, we are available 24/7 to ensure you never miss a single important story.</p>`,
  'ja': `<h2>Demo.Websiteについて</h2>
<p>Demo.Websiteは、インドネシアおよびその周辺の読者に最新かつ正確で公平な情報を提供することに特化した、信頼できるデジタルニュースポータルです。</p>
<h3>私たちのビジョン</h3>
<p>高品質で責任あるジャーナリズムを通じて社会を啓発する、信頼できる情報源になること。</p>
<h3>私たちのミッション</h3>
<p>検証されたニュースを提供し、誠実さの原則を守り、すべての報道においてさまざまな視点のための場を提供することに取り組んでいます。</p>
<h3>編集チーム</h3>
<p>ジャーナリズムや様々な専門分野のバックグラウンドを持つ経験豊富なジャーナリストチームが支え、重要なニュースを一つも見逃さないよう24時間365日対応しています。</p>`,
  'es': `<h2>Sobre Demo.Website</h2>
<p>Demo.Website es un portal de noticias digital confiable dedicado a brindar información actualizada, precisa y equilibrada a los lectores de toda Indonesia y más allá.</p>
<h3>Nuestra Visión</h3>
<p>Convertirnos en una fuente de información confiable que empodere a la sociedad a través de un periodismo de alta calidad y responsable.</p>
<h3>Nuestra Misión</h3>
<p>Estamos comprometidos a presentar noticias verificadas, defender los principios de honestidad y brindar espacio para diversas perspectivas en cada reportaje.</p>
<h3>Equipo Editorial</h3>
<p>Fortalecidos por un equipo de periodistas experimentados con formación en periodismo y diversos campos de especialización, estamos disponibles 24/7 para asegurarnos de que nunca te pierdas una sola historia importante.</p>`,
  'zh': `<h2>关于 Demo.Website</h2>
<p>Demo.Website 是一个值得信赖的数字新闻门户，致力于为印度尼西亚及其他地区的读者提供最新、准确和均衡的信息。</p>
<h3>我们的愿景</h3>
<p>通过高质量和负责任的新闻业，成为赋予社会力量的可信信息来源。</p>
<h3>我们的使命</h3>
<p>我们致力于呈现经过核实的新闻，坚守诚信原则，并在每篇报道中为多元视角提供空间。</p>
<h3>编辑团队</h3>
<p>由具有新闻学和各专业领域背景的经验丰富的记者团队支撑，我们全天候24/7运营，确保您不会错过任何一条重要新闻。</p>`
};

// Helper: get localized title/content for a news item
function getNewsLocalized(n) {
  if (currentLang !== 'id' && n.translations && n.translations[currentLang]) {
    return { title: n.translations[currentLang].title, content: n.translations[currentLang].content };
  }
  return { title: n.title, content: n.content };
}

// Helper: get localized about content
function getAboutLocalized() {
  if (currentLang !== 'id' && aboutTranslations[currentLang]) return aboutTranslations[currentLang];
  return aboutContent;
}
...
function renderHomeNews() {
  ...
  container.innerHTML = latest.map(n => {
    const loc = getNewsLocalized(n);
    const excerpt = loc.content.replace(/\n+/g, ' ').substring(0, 120) + '...';
    const img = n.image_url || `https://picsum.photos/seed/${n.id}/800/450`;
    return `<div class="latest-card" onclick="showArticle(${n.id})">
      <img src="${img}" alt="${escapeHtml(loc.title)}" loading="lazy" onerror="this.src='https://picsum.photos/seed/${n.id}x/800/450'">
...
function showArticle(id) {
  ...
  document.getElementById('article-content').innerHTML = `
    ...
    <h1 class="article-title">${escapeHtml(loc.title)}</h1>
    ...
    ${news.image_url ? `<img class="article-cover" src="${news.image_url}" alt="${escapeHtml(loc.title)}" onerror="this.style.display='none'">` : ''}
    ...
    <div class="news-actions" style="margin-bottom:24px">
      ...
      <button class="action-btn" onclick="toggleComments(${news.id},this)">
        ...
        <span id="comment-count-${news.id}">${news.comment_count}</span> ${commentLabel}
      </button>
    </div>
    <div class="comment-section open" id="comments-${news.id}">
      <div class="comment-form">
        <input class="comment-input" type="text" id="comment-input-${news.id}" placeholder="${commentPh}" maxlength="500">
        <button class="btn-comment-submit" onclick="submitComment(${news.id})">${sendLabel}</button>
      </div>
...
function renderNewsCard(n) {
  ...
  <article class="news-card" data-id="${n.id}">
    <div class="news-card-img-wrap">
      <img class="news-card-image" src="${imgSrc}" alt="${escapeHtml(loc.title)}" loading="lazy"
    ...
    <div class="news-card-body">
      ...
      <h2 class="news-card-title" onclick="showArticle(${n.id})">${escapeHtml(loc.title)}</h2>
...
function renderAbout() {
  document.getElementById('about-content').innerHTML = getAboutLocalized();
...
function renderAdminNews() {
  ...
  list.innerHTML = newsData.map(n => {
    ...
    <div class="admin-news-item" id="admin-item-${n.id}">
      <div class="admin-news-item-title">${escapeHtml(loc.title)}</div>
...
function submitLogin() {
  ...
  setTimeout(() => {
    ...
    if (user && user.password === password) {
      ...
      localStorage.setItem('np_session', JSON.stringify(currentUser));
      updateNavUser();
      closeAuthModal();
      const greeting = tGet('login_greeting') || 'Selamat datang, ';
      showToast(`${greeting}${username}!`, 'success');
...
function syncDrawerActive(page) {
  document.querySelectorAll('.nav-drawer ul li a').forEach(a => a.classList.remove('active'));
  const el = document.getElementById(`drawer-nav-${page}`);
...
function updateDrawerUser() {
  ...
  if (!bottom) return;
  const drawerAdminLi = document.getElementById('drawer-admin-li');
  if (drawerAdminLi) drawerAdminLi.style.display = (currentUser && currentUser.role === 'admin') ? 'block' : 'none';

  if (currentUser) {
    const badge = currentUser.role === 'admin'
      ? `<span class="admin-badge">${tGet('nav_admin_badge') || 'Admin'}</span>` : '';
    bottom.innerHTML = `
      <div class="nav-drawer-user">
        <div class="user-avatar">${currentUser.username[0].toUpperCase()}</div>
        <span class="user-name">${currentUser.username}</span>${badge}
      </div>
      <button class="btn-logout" onclick="logout();closeDrawer()" data-t="nav_logout">Keluar</button>`;
  } else {
    bottom.innerHTML = `
      <button class="btn-login" onclick="openAuthModal();closeDrawer()">
...
function setLanguage(lang) {
  ...
  if (currentPage === 'article') {
    // re-render current article in new language
    const articleEl = document.getElementById('article-content');
    const backBtn = articleEl ? articleEl.querySelector('.article-back') : null;
    if (backBtn) {
      // find the current article id from the like button
      const likeBtn = articleEl.querySelector('[id^="like-btn-"]');
      if (likeBtn) {
        const id = parseInt(likeBtn.id.replace('like-btn-', ''));
        showArticle(id);
      }
    }
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    ...
    if (saved) {
      ...
      if (parsed && USERS[parsed.username] && parsed.role) {
        currentUser = parsed;
      }
    }
  } catch(e) { localStorage.removeItem('np_session'); }
  ...
  // If session restored, update nav before first navigate
